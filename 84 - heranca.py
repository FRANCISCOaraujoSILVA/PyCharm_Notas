...
em algum lugar do código - os sinalizadores (azuis) do lado direito do número das linhas que ficam ali do lado esquerdo
- Quando uma classe herda de outra classe, todos os atributos e métodos dessa classe são herdados
...
- Com o método super() podemos fazer acesso a qualquer atributo é método de uma Super Classe. Em nosso caso, estamos
...
A pergunta que deve ser feita: Existe alguma entidade genérica o suficiente para encapsular os atributos e métodos 
...
class NovoCliente(Pessoa):  # NovoCliente herda de Pessoa
    def __init__(self, nome, sobrenome, cpf, renda):
        ...
        # Pessoa.__init__(self, nome, sobrenome, cpf)  # Também poderia ser assim, mas não é a forma comum de acessar os
...
# Veja a quantidade de códigos que foram economizados
...
também sobre a bolinha do método nome_completo() na classe Pessoa.
    - Isso ocorre quando reescrevemos/reimplementamos um método presente na super classe em classes filhas
"""
